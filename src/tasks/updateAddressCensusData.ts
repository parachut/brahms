...
export async function updateAddressCensusData(req, res) {
  ...
  if (addressId && req.header('X-AppEngine-TaskName')) {
    ...
    try {
      ...
      if (result.fields && result.fields.census) {
        ...
        if (exists) {
          ...
          forIn(incomeRange, function(value, key) {
            ...
            if (range && range.length === 2) {
              const nRange = range.map((n) => Math.round(Number(n)));
              const existing = incomeRanges.find(
                (r) => r.range[0] === nRange[0],
              );
              if (!existing) {
                incomeRanges.push({
                  range: nRange,
      ...
      return res.send(`Address information collected: ${address.id}`).end();
    } catch (e) {
      console.log(e);
    }
  }
